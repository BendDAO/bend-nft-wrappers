...
import { deployNewImplmentation, getContractAddressFromDB } from "./utils/helpers";
...
task("prepareUpgrade", "Deploy new implmentation for upgrade")
  ...
  .setAction(async ({ proxyid, implid, skipcheck }, { ethers, upgrades, run }) => {
    await run("set-DRE");
    await run("compile");
...
task("newImplmentation", "force import implmentation to proxy")
  .addParam("implid", "The new impl contract id")
  .setAction(async ({ implid }, { ethers, upgrades, run }) => {
    await run("set-DRE");
    await run("compile");

    const impl = await deployNewImplmentation(implid, true);

    console.log(`New Implmentation: ${impl.address}`);
  });
